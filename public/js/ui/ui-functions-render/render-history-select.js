...
export function renderHistorySelect(filepath, entries) {
    ...
    return `<button><span class="opt-filename" data-prop="filename">${filepath}&nbsp;</span><span class="opt-version"></span></button>`+
           `<option value="current" class="flex-row"><span class="opt-time">current version</span><span class="flexgrow"></span><button data-action="file-options-open" id="file-options-btn">file options</button></option>` +
           historyOptions;
}
